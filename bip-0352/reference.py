...
def create_outputs(input_priv_keys: List[Tuple[ECKey, bool]], input_hash: bytes, recipients: List[str], hrp="tsp", permute=True) -> List[str]:
    G = ECKey().set(1).get_pubkey()
    negated_keys = []
    ...
    for B_scan, B_m_values in silent_payment_groups.items():
        ...
        # Order doesn't matter for creating/finding the outputs. However, different orderings of the recipient addresses
        # will produce different generated outputs if sending to multiple silent payment addresses belong to the
        ...
        if permute:
            for permutation in permutations(B_m_values, len(B_m_values)):
                k = 0
                for B_m in permutation:
    ...
    for case in test_data:
        ...
        for sending_test in case["sending"]:
            ...
            if (len(input_pub_keys) > 0):
                ...
                sending_outputs = create_outputs(input_priv_keys, input_hash, given["recipients"], hrp="sp", permute=False)
                # Check that for a given set of inputs, we were able to generate the expected outputs for the receiver
            for output in sending_outputs:
                assert output in expected["outputs"], "Sending test failed"
